...
if (post("nomor")) {
    ...
    if (empty($cek->num_rows)) {
        toastr_set("error", _USERNAME_PHONE_NOT_MATCH);
    } else {
		$username = getSingleValDB("device", "nomor", "$nomor", "pemilik");
		$otp = rand(123,789);
		$password = sha1($otp);
        $pesan = 
                            ...
                            <div class="col-12">
                                <div class="p-5">
                                    <div class="text-center">
                                        <h1 class="h4 text-gray-900 mb-4"><?php echo _PASSWORD_RESET ?></h1>
                                    </div>
                                    <form class="user" method="POST">
                                        ...
                                        <div class="form-group">
                                            <input type="tel" class="form-control form-control-user" id="nomor" placeholder="<?php echo _PHONE_INCLUDE_COUNTRY ?>" name="nomor" required>
                                        </div>
                                        <button type="submit" class="btn btn-primary btn-user btn-block" onclick="return confirm('<?php echo _PASSWORD_RESET_CONFIRM ?>')">
                                            <?php echo _PASSWORD_RESET ?>
                                        </button>
                                    </form>
                                    <hr>
                                    <div class="text-center">
